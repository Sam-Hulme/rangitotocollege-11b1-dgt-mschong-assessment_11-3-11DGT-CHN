...
# A grid of empty strings that will be filled to reflect the types of objects at each location
objects = [[]]
panelWidth = 0
panelHeight = 0
...
def init(columns, rows, width, height):
    for i in range(columns*rows):
        row = len(panels)-1
        # if the current row list would have more items than the desired column count, move to the next row
        if (len(panels[row])+1 > columns):
            panels.append([])
            objects.append([])
        row = len(panels)-1
        column = len(panels[row])
        panel = Canvas(root, width=width, height=height,
                       bg="black", highlightthickness=0, bd=0)
        panel.grid(row=row, column=column)
        # Add all panels to a list (so they can be iterated through)
        panels[row].append(panel)
        objects[row].append(["", "", ""])
        global panelWidth
        global panelHeight
        panelWidth = width
    ...
    for y in panels:
        for x in y:
            ...
            x.create_rectangle(0, 0, panelWidth, panelHeight,
                               fill="", outline="white", tags="debug")
            x.create_text(panelWidth/2, panelHeight/2,
                          text=f"{row}, {column}", fill="white", tags="debug")
...
def _create_circle(self, x, y, r, **kwargs):
    """Create a circle based on a center point and radius rather than two corners like the original oval function."""
