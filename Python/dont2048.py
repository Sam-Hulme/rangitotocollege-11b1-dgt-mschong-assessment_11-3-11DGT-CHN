...
def move(event):
    ...
    if len(numbers) == 16:
        ...
        for n in numbers:
            ...
            for i in adjacent:
                # If it is trying to check a number outside the screen
                if (i[0] in [-1, 4] or i[1] in [-1, 4]):
                    continue  # Skip this number and check the next one
                # If the number is the same as the current number
                if objects[i[0]][i[1]][0] == n[2]:
                    # print("Not stuck")
...
goals = []
# A list of extra objects (the text and buttons for game over or victory)
extras = []
...
def reset():
    ...
    goals = []
    for i in range(4):
        goal = Canvas(root, width=42, height=42,
        ...
        goal.circle = goal.create_oval(
            2, 2, 40, 40, outline=info[i][1], width=3)
        goal.create_text(21, 21, anchor=CENTER, text=info[i][0], fill='white', font=(
            'Arial', 12, 'bold'), justify=CENTER)
        goals.append(goal)
    global currentGoal
    global score
    global highestNumber
...
def end(fail):
    ...
    if fail:
        ...
        goals[currentGoal -
              5].itemconfig(goals[currentGoal-5].circle, fill='green')
    else:
        title = "Game Over!"
        subtitle = "You did 2048!"
    # Create a frame to make the game over text a fixed height.
    frame = Frame(root, width=panelWidth*4, height=30, bg='black')
