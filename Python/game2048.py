...
def end(fail):
    ...
    if fail:
        title = "Game Over!"
        subtitle = f'Highest Number: {highestNumber}'
    else:
        title = "Victory!"
    ...
    if fail:
        ...
        restart = Button(root, text='Continue', command=cont)
    extras.append(restart)
    end = Button(root, text='Return', command=exit)
    extras.append(end)
    restart.grid(row=6, column=0, columnspan=2, sticky='nsew')
    end.grid(row=6, column=2, columnspan=2, sticky='nsew')
    root.unbind('<Key>')
