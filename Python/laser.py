...
def emitterSprite(y,x,**data): 
    ...
    if colour == False and active == False:
        panels[y][x].bind("<Button-1>", lambda event: activateDefault(y,x,dir))
    else:
...
def doorSprite(y,x,**data):
    ...
    if not open:
        ...
        objects[y][x] = ['','',''] #If the door is open, clear the object data so it behaves like an empty panel
...
def boxSprite(y,x,**data):
    ...
    if stage == 5:
        ...
        if not spawner: #if spawner is the default spawner (default spawner is False)
    ...
    if stage == 0:
        ...
        objects[y][x][0] = 'w' #Set the object type to a wall so that lasers are blocked (this doesn't affect the sprite and removes the need to add a fourth data item for breaking state)
    ...
    if selectedObject == [y,x] and panels[y][x].find_withtag('selected') == () and panels[y][x].find_withtag('frame') == () and stage == 0: #If there is no object with the 'selected' tag (so no selection indicator), no object with the 'frame' tag (no button frame), this box is selected, and this box isn't breaking, create an indicator 
        selectIndicator()
...
def boxSpawnerSprite(y,x,**data):
    ...
    if open > 0 and (not active or not objects[y][x][0] == 's'): #If the spawner is inactive or isnt replacing another spawner, reset open to 0
        open = 0
        ...
        for objY in range(10):
            ...
            openAnimEvent.append(panels[y][x].after(500,lambda: boxSpawnerSprite(y,x,open=1,colour=colour,active=True))) #If there is no box, spawn one.
    ...
    if open == 1:
        openAnimEvent.append(panels[y][x].after(750,lambda: boxSpawnerSprite(y,x,open=2,colour=colour,active=True)))
    if open == 2:
        ...
        openAnimEvent.append(panels[y][x].after(1000,lambda: boxSprite(y,x,spawner=colour)))
...
def laserUpdate():
        ...
        while True:
                ...
                try:
                    if (x < 0 or y < 0):
                        ...
                        boxSprite(y,x,stage=1,spawner=objects[y][x][1]) #If the laser collides with a box, start breaking it.
                        break
                    else:
    ...
    def updateColours():
        for i in colours:
            try:
                if not laserRecievers[i][3]: #If the reciever is not active after updating the laser
                    colours[i] = False #Set the colour value in the dictionary to false
    ...
    for c, i in laserRecievers.items():
        recieverSprite(i[0],i[1],dir=i[2],colour=c) #Deactivate all reciever sprites (does not deactivate colours)
    yLoop = 0
    xLoop = 0
    ...
    for y in range(10):
        for x in range(10):
            if objects[y][x][0] == 'n' or objects[y][x][0] == 'b': #Also update box button functions
                type = objects[y][x][0]
...
def objectMove(event, object):
    ...
    x = selectedObject[1]
    y = selectedObject[0]
    if objects[y][x][0] == 'w' or objects[y][x][0] == 's': #If the object is a box that is breaking or a spawner spawning a box
        return
    type = objects[y][x][0]
    dir = objects[y][x][1]
    ...
    # print(objects[y][x])
    if oldType not in ['','l']: 
        setPanel(y,x,oldType,colour=oldData2,dir=oldData1,flipped=oldData1,open=oldData1) #add any extra data kwargs here for objects that can be moved over
    else:
        panels[y][x].delete("main")
        objects[y][x] = ['','','']
